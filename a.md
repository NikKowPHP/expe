<reasoning>
I will refine the Analytics page to separate Income and Expenses and add a Savings Rate metric.

Plan:
1.  **Update `app/(dashboard)/analytics/page.tsx`**:
    *   Add state `viewType` ('expense' | 'income').
    *   Separate filtered transactions into `incomeTransactions` and `expenseTransactions`.
    *   Calculate `totalIncome`, `totalExpense`, and `savingsRate`.
    *   Add a toggle switch for the charts.
    *   Add a Summary Card showing Total Income, Total Expense, and Savings Rate.
    *   Update Pie Chart and Bar Charts to reflect the selected `viewType`.
</reasoning>
